...
function showGallery(id) {
  ...
  Object.keys(galleries).forEach(function(key) {
              ...
              if (section) section.style.display = (key === id) ? 'block' : 'none';
  });
  ...
  if (grid.dataset.built === '1') {
              lightboxPhotos = grid.dataset.photos ? JSON.parse(grid.dataset.photos) : [];
              return;
  }
  ...
  grid.dataset.built = '1';
          grid.dataset.photos = JSON.stringify(lightboxPhotos);
}
