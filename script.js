...
function buildGrid(id) {
  ...
  // Build list of src paths
  var srcs = [];
    for (var i = 1; i <= count; i++) {
          srcs.push(folder + '/' + String(i).padStart(3, '0') + '.jpg');
    }

  // Show a loading indicator while preloading dimensions
  grid.innerHTML = '<p class="grid-loading">Loading\u2026</p>';

  // Preload all images to get natural dimensions
  var loaded = 0;
    var dims = new Array(count);
    var total = count;

  srcs.forEach(function(src, idx) {
        var probe = new Image();
        probe.onload = function() {
                dims[idx] = { w: probe.naturalWidth, h: probe.naturalHeight };
                loaded++;
                if (loaded === total) renderGrid(grid, id, srcs, dims);
        };
        probe.onerror = function() {
                dims[idx] = { w: 4, h: 3 }; // fallback ratio
                loaded++;
                if (loaded === total) renderGrid(grid, id, srcs, dims);
        };
        probe.src = src;
...
function renderGrid(grid, id, srcs, dims) {
    grid.innerHTML = '';
  ...
  srcs.forEach(function(src, idx) {
        ...
        var dim = dims[idx];
        var paddingBottom = ((dim.h / dim.w) * 100).toFixed(4) + '%';

                   var item = document.createElement('div');
        item.className = 'photo-item';
        item.dataset.index = idx;

                   // Aspect-ratio wrapper keeps exact space reserved before image loads
                   var wrapper = document.createElement('div');
        wrapper.className = 'photo-ratio';
